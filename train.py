...
from model import SNN_BiLSTM, Normal_TextCNN, CombinedModel
import numpy as np
...
def build_dataloader(args: SNNArgs, dataset, split='train'):
    ...
    setattr(args, f'{split}_dataloader', DataLoader(dataset, batch_size=args.batch_size, shuffle=True, drop_last=True))
    # if split == 'train':
...
def build_model(args: SNNArgs):
    ...
    if args.model_mode == "ann":
        ...
        if args.model_type == "cnn_snn":
            # combine textcnn and snn_bilstm
            # 加载预训练模型
            pretrained_snn_lstm = SNN_BiLSTM(args)
            pretrained_snn_lstm.load_state_dict(torch.load(args.pretrained_snn_lstm_path))
            pretrained_snn_lstm.to(args.device)
            pretrained_cnn = Normal_TextCNN(args)
            pretrained_cnn.load_state_dict(torch.load(args.pretrained_cnn_path))
            pretrained_cnn.to(args.device)
            # 冻结预训练模型的参数
            for param in pretrained_snn_lstm.parameters():
                param.requires_grad = False
            for param in pretrained_cnn.parameters():
                param.requires_grad = False
            # 初始化组合模型
            args.model = CombinedModel(pretrained_snn_lstm, pretrained_cnn, args)
...
def build_optimizer(args: SNNArgs):
    ...
    if args.optimizer_name == "Adamw":
        if args.model_mode == "combine":
            args.optimizer = AdamW(args.model.fc_combined.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay, betas=(0.9, 0.999))
        else:
...
if __name__ == "__main__":
    ...
    if args.mode == 'train' and args.model_mode == "snn":
        ...
        ann_train(args)
    elif args.mode == 'train' and args.model_mode == "combine":
        ann_train(args)
